...
using UnityPlugins.Input.Unity;
using UnityEditor;
using UnityEngine;
...
namespace UnityPlugins.Input.Editor
{
	...
	public class InputManagerEditor : AExtendedEditor<InputManagerComponent>
	{
		...
		{
			IReadOnlyList<InputLayer> activeLayers = Target.Manager.Receivers.ActiveLayers;
		...
		private void DrawMouseState(MouseDevice mouse)
		{
			...
			EditorGUILayout.Vector2Field("Raw Delta", mouse.NormalizedDelta);
		...
		private void DrawDeviceInputState(string name, AInputDevice device, int minLineCount)
		{
